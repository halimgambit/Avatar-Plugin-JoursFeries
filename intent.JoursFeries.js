...
exports.default = function (state, actions) {
	...
	var match;
	for (var rule in Config.modules.JoursFeries.rules) {	 
		match = (0, _helpers.syntax)(state.sentence, Config.modules.JoursFeries.rules[rule]); 	
		if (match) break;
	}
	
	if (match) { // vérifié !
		if (state.debug) info('Intent JoursFeries, Syntax: true');
		state.isIntent = true;
